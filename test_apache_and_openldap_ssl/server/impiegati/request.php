...
$result = ldap_search($conn,"dc=marius,dc=com", "(&(uid=".$name.")(userpassword=".$password.")(ou=impiegato))");
...
;
if($cn != $entry["cn"][0]){
	die("Certificate e user name non non corrispondono");
}
else 
	{
		$cert_cred=true;
...
if($email != $entry["mail"][0]){
	die("Certificate e email does non corrispondono");
}
	else {
		$cert_cred=true;
	}
